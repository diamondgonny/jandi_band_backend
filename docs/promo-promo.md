...
- **팀명**: teamName용으며 모든 공연 홍보에 사용
- **이미지**: 인증/수정 관련 처리, 1개만 허용
- **이미지 형식**: JPG, PNG 등, 일반적인 이미지 형식 지정
- **이미지 교체**: 수정 시 새 이미지로 하면서기존 이미지 동시 교체
- **부분 수정**: PATCH 방식으로 전송하면 전송되지 않은 필드는 기존 값이 유지됩니다.
- **동기 계산**: viewCount, commentCount, likeCount 동기 관련
- **프로그램 종료**: 제 이외에 deletedAt 정, 이미지관련 처리 후 삭제
- **주의**: 수정/삭제 후 최신 데이터 확인
- **좋아요**: 같은 API로 작성 (같은 이미지 추가/취소), 중복 좋아요 방지
- **좋아요 상태**: 공연 홍보 목록/상세 조회 시 `isLikedByUser` 필드 포함 (true: 좋아요 누름, false: 좋아요 안 누름, null: 인증되지 않은 사용자)
- **정렬 션**: `sort` 파라미터로 정렬 기준 지정가능(기본값 `createdAt,desc`)
- **응답 책**: 
  - **성공**: 성공 시 리소스의 ID반환 (성능 최적화)
  - **정보**: 간단한 성공 메시지반환
  - **에러**: 간단한 에러 메시지반환
  - **세 정보 필요 경우**: 별도 조회 API 출최신 페이지인
- **추가 API**: 관련 기능 별도 참조
