...
public class PollController {
    ...
    @PostMapping
    public ResponseEntity<CommonResponse<PollRespDTO>> createPoll(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(CommonResponse.success("투표가 성공적으로 생성되었습니다.", responseDto));
    }

    @Operation(summary = "클럽별 투표 목록 조회")
    @GetMapping("/clubs/{clubId}")
    public ResponseEntity<CommonResponse<Page<PollRespDTO>>> getPollList(
            @PathVariable Integer clubId,
            @PageableDefault(size = 5) Pageable pageable) {
        Page<PollRespDTO> polls = pollService.getPollsByClub(clubId, pageable);
        return ResponseEntity.ok(CommonResponse.success("투표 목록을 조회했습니다.", polls));
    }

    @Operation(summary = "투표 상세 조회")
    @GetMapping("/{pollId}")
    public ResponseEntity<CommonResponse<PollDetailRespDTO>> getPollDetail(
        ...
        return ResponseEntity.ok(CommonResponse.success("투표 상세 정보를 조회했습니다.", responseDto));
    }

    @Operation(summary = "투표에 곡 추가")
    @PostMapping("/{pollId}/songs")
    public ResponseEntity<CommonResponse<PollSongRespDTO>> addSongToPoll(
            @PathVariable Integer pollId,
